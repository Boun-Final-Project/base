...
Numerical contract with SpatialObsWrapper:
    - Same five channels in the same order:
      [is_known, is_wall, gas, recency, det_count/max]
...
class BatchedObsBuilder:
    ...
    def update_batch(self, robot_pos_np, binary_gas_np, decay_mask=None):
        ...
        # 1. Recency decay (skip freshly-reset envs)
        ...
        # 3. Robot-cell point updates (gas, rec, det, max_det)
    ...
    def build_obs(self, robot_pos_np, wind_raw_np, current_step_np):
        """Return (spatial (N, 5, G, G), wind (N, 3)) on device."""
        N   = self.N
        G   = self.GRID_SIZE
        dev = self.device
        ...
        spatial = torch.stack([known, wall, gas, rec, det], dim=1)  # (N, 5, G, G)
