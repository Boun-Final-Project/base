...
if __package__ is None or __package__ == "":
    ...
    from reinforcement_learning.envs.batched_obs_builder import BatchedObsBuilder
    ...
    from ..envs.batched_obs_builder import BatchedObsBuilder
...
def _stack_spatial(obs_list):
    return (np.stack([o[0] for o in obs_list]),   # (N, 5, 98, 98)
            np.stack([o[1] for o in obs_list]))    # (N, 3)
...
# ---------------------------------------------------------------------------
# Batched (GPU-side) spatial obs path
# ---------------------------------------------------------------------------

def _thin_worker(child_conn, parent_conn, env_fn):
    """Worker that owns a raw GasSourceEnv and sends back only scalar state.

    Payload per step (~dozens of bytes) instead of a 192 KB spatial obs — all
    obs-building happens on the main process inside BatchedObsBuilder.
    Includes the new GT grid on reset so main can refill builder state.
    """
    parent_conn.close()
    env = env_fn()
    ...
    def _attach_reset_state(snap):
        snap["gt_grid"] = env._grid.grid.copy()
        snap["map_h"]   = int(np.floor(env._map_height / cfg.VISITED_CELL_RESOLUTION))
        snap["map_w"]   = int(np.floor(env._map_width  / cfg.VISITED_CELL_RESOLUTION))
        ...
        while True:
            ...
            if cmd == "step":
                ...
                if done:
                    ...
                    info["truncated"]  = truncated
                    _, _ = env.reset()
                    snap = _state_snapshot(reward, True, info)
                    # For freshly-reset env, gas reading resets to 0 regardless
                    snap["gas"] = False
                    _attach_reset_state(snap)
                else:
                    snap = _state_snapshot(reward, False, info)
                child_conn.send(snap)
            elif cmd == "reset":
                _, info = env.reset()
                snap = _state_snapshot(0.0, False, info)
                snap["gas"] = False  # reset semantics — match SpatialObsWrapper
                _attach_reset_state(snap)
                child_conn.send(snap)
            elif cmd == "set_curriculum":
                w_range, h_range, max_template = data
...
class BatchedSpatialVecEnv:
    """Main-process GPU obs-builder + subprocess workers running raw envs.

    Thin workers send ~30 byte packets per step (robot_pos + scalars);
    main process batches reveal + ego embedding on the GPU.
    """
    ...
    def __init__(self, env_fns, device):
        ...
        self._procs = []
        for child_conn, parent_conn, fn in zip(child_conns, self._parents, env_fns):
            p = ctx.Process(target=_thin_worker,
                            args=(child_conn, parent_conn, fn),
                            daemon=True)
            p.start()
            child_conn.close()
        ...
        self.device  = device
        # Probe one env for its true-grid resolution (workers already constructed
        # their envs so this doesn't affect them).
        probe = GasSourceEnv()
        probe.reset(seed=0)
        self.builder = BatchedObsBuilder(num_envs=self.num_envs, device=device,
                                         true_res=probe._grid.resolution)

    def _apply_snapshots(self, snaps):
        """Fold worker snapshots into the GPU builder + return batched obs."""
        N = self.num_envs
        robot_pos = np.stack([s["robot_pos"] for s in snaps])                # (N, 2)
        wind_raw  = np.stack([s["wind_raw"]  for s in snaps])                # (N, 3)
        step_arr  = np.array([s["step"]  for s in snaps], dtype=np.int64)    # (N,)
        gas_arr   = np.array([s["gas"]   for s in snaps], dtype=bool)        # (N,)
        rewards   = np.array([s["reward"]for s in snaps], dtype=np.float32)
        dones     = np.array([s["done"]  for s in snaps], dtype=np.float32)
        infos     = [s["info"] for s in snaps]

        # Refill builder state for any freshly-reset envs
        for i, s in enumerate(snaps):
            if s["gt_grid"] is not None:
                self.builder.reset_env(
                    env_idx=i,
                    gt_grid_np=s["gt_grid"],
                    map_h_cells=s["map_h"],
                    map_w_cells=s["map_w"],
        ...
        self.builder.update_batch(robot_pos, gas_arr)
        spatial, wind = self.builder.build_obs(robot_pos, wind_raw, step_arr)
        return (spatial, wind), rewards, dones, infos

    def reset(self):
        for conn in self._parents:
            conn.send(("reset", None))
        snaps = [conn.recv() for conn in self._parents]
        (spatial, wind), _, _, infos = self._apply_snapshots(snaps)
        return (spatial, wind), infos
...
def train(args):
    ...
    if args.batched_obs:
        assert is_spatial, "--batched-obs only applies to --arch spatial"
        assert not args.serial, "--batched-obs requires subprocess workers"
        # Thin workers run raw GasSourceEnv; obs-building happens on GPU in main.
        raw_fns = [make_env(args.seed, i, template_id=template_id) for i in range(args.num_envs)]
        vec_env = BatchedSpatialVecEnv(raw_fns, device=device)
        print(f"VecEnv: batched-GPU obs ({args.num_envs} thin workers, {mp.cpu_count()} CPU cores)")
    elif args.serial:
    ...
    for update in range(start_update + 1, num_updates + 1):
        ...
        # Entropy coefficient annealing (linear from --entropy-coeff to --entropy-coeff-end)
        if args.entropy_coeff_end is not None:
            progress = (update - 1) / num_updates
            current_ent_coeff = args.entropy_coeff + progress * (args.entropy_coeff_end - args.entropy_coeff)
        else:
            current_ent_coeff = args.entropy_coeff
        ...
        for step in range(args.rollout_length):
            ...
            with torch.no_grad():
                if is_spatial:
                    sp0, w0 = obs
                    if torch.is_tensor(sp0):
                        spatial_t = sp0.to(dtype=torch.float32)
                        wind_t    = w0.to(dtype=torch.float32)
                    else:
                        spatial_t = torch.tensor(sp0, dtype=torch.float32, device=device)
                        wind_t    = torch.tensor(w0,  dtype=torch.float32, device=device)
        ...
        stats = _update_fn(
            ...
            args.clip_epsilon, current_ent_coeff, args.value_loss_coeff,
...
def main():
    ...
    parser.add_argument("--batched-obs", action="store_true", default=False,
                        help="Build spatial obs on GPU from scalar worker snapshots "
                             "(spatial arch only; thin workers + main-process batched reveal)")
    ...
    parser.add_argument("--entropy-coeff-end", type=float, default=None,
                        help="Final entropy coeff for linear annealing from --entropy-coeff. "
                             "None = no annealing (constant coeff).")
