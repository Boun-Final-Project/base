...
class SpatialRolloutBuffer:
    """Rollout buffer for spatial (C, 98, 98) + ctx (K,) observations.

    Spatial and ctx tensors are kept on CPU — at the defaults (1024 × 8 envs ×
    5 × 98² × 4 B ≈ 1.5 GB) this avoids the same-sized GPU allocation.
    Minibatches are moved to device on demand inside get_batches().
    """

    def __init__(self, num_steps, num_envs, action_dim, device,
                 spatial_channels=6, ctx_dim=3):
        self.num_steps = num_steps
        self.num_envs  = num_envs
        self.device    = device
        self.pos       = 0
        ...
        self._K = ctx_dim
        self.spatial_obs = torch.zeros((num_steps, num_envs, self._C, G, G),
                                       dtype=torch.float32)
        self.wind_obs    = torch.zeros((num_steps, num_envs, self._K),
                                       dtype=torch.float32)
    ...
    def get_batches(self, advantages, returns, num_minibatches):
        ...
        b_spatial    = self.spatial_obs.reshape(batch_size, self._C, self._G, self._G)
        b_wind       = self.wind_obs.reshape(batch_size, self._K)
        b_actions    = self.actions.reshape(batch_size, -1)
        b_log_probs  = self.log_probs.reshape(batch_size)
        b_advantages = advantages.reshape(batch_size)
        b_returns    = returns.reshape(batch_size)
