...
Input:
    spatial : (B, 6, 98, 98) — [is_known, is_wall, gas, recency, det_count, motion]
...
CNN output geometry (98×98 input, 0.5 m/cell):
    ...
    Dynamic stream (3+1 = 4 ch)        → 13×13×64
    1×1 fusion                         → 13×13×128  → flatten → 21632

The 6th channel (motion trail) is fed into the dynamic stream because it
varies on a per-step basis — same temporal-character family as gas, recency,
det_count. Existing 5-channel checkpoints can be loaded via the converter
in `models/convert_5ch_to_6ch.py`, which zero-initialises the new input
slot of dynamic_cnn[0] so the policy at init is bit-identical to the old.
"""
...
class ActorCriticSpatial(nn.Module):
    ...
    def __init__(
        ...
        # Dynamic stream: gas + recency + det_count + motion (channels 2-5)
        self.dynamic_cnn = nn.Sequential(
            layer_init(nn.Conv2d(4,  16, 5, stride=2, padding=2)), nn.ReLU(),  # 49×49
            layer_init(nn.Conv2d(16, 32, 3, stride=2, padding=1)), nn.ReLU(),  # 25×25
    ...
    def _encode(self, spatial, wind):
        ...
        dynamic = self.dynamic_cnn(spatial[:, 2:6])                        # (B, 64, 13, 13)
