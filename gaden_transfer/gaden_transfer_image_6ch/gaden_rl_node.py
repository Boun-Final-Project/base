"""
GadenRLNodeOsl — deploy the 5-channel spatial PPO agent inside GADEN.
...
Usage
-----
    ros2 run reinforcement_learning gaden_rl_node_osl --ros-args \
...
class GadenRLNodeOsl(Node):

    def __init__(self):
        super().__init__('gaden_rl_node_osl')
        ...
        self.get_logger().info(f'GadenRLNodeOsl ready. Checkpoint: {self._checkpoint_path}')
    ...
    def _load_agent(self):
        ...
        if self._use_nav2:
            ...
            self.get_logger().info(
                ...
                f'Waiting up to 60s for /PioneerP3DX/navigate_to_pose action server...'
            )
            # Pre-flight: wait WITH a timeout. Navigator() inside efe_igdm
            # calls wait_for_server() with no timeout, so a missing/stale
            # Nav2 lifecycle would hang us forever (we've seen 1-in-5 batch
            # runs hit this when DDS state from a previous run lingers).
            # Bail out fast so the batch script can move on.
            _pre = ActionClient(self, NavigateToPose, '/PioneerP3DX/navigate_to_pose')
            if not _pre.wait_for_server(timeout_sec=60.0):
                self.get_logger().error(
                    'Nav2 action server not available after 60s — aborting. '
                    '(Likely DDS-discovery race; the batch script will record '
                    'this as crashed_or_unknown and proceed to the next run.)'
                )
                _pre.destroy()
                os._exit(2)
            _pre.destroy()
    ...
    def _is_valid_slam(self, position, radius=cfg.ROBOT_RADIUS):
        ...
        Out-of-bounds cells inside the robot footprint are treated as walls
        (return False). Otherwise a target near the map edge can pass the
        check while the footprint pokes off-map, and basic_sim's loose
        boundary enforcement lets the robot drift outside — once outside,
        every retreat target also has an off-map footprint, so the robot
        ...
        if not (0 <= gx < gmap.grid_width and 0 <= gy < gmap.grid_height):
            return False  # target centre out-of-map → blocked
        ...
        for ddx in range(-rc, rc + 1):
            for ddy in range(-rc, rc + 1):
                if ddx * ddx + ddy * ddy > rc_sq:
                    continue
                cx, cy = gx + ddx, gy + ddy
                # Footprint extending off-map → treat as wall (matches
                # physical reality: a real robot can't drive past the wall).
                if not (0 <= cx < gmap.grid_width and 0 <= cy < gmap.grid_height):
                    return False
