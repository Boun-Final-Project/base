...
# GADEN DENSE-GAS Preprocessing + Simulation Launcher  (predensegas)
# Interactive menu — same UX as presim, but runs the 3-STEP DENSE-GAS recipe:
#   1) GADEN_FILL_WIND=0 preprocess  -> faulty sawtooth wind
#   2) filament sim                  -> bake the DENSE gas on that wind
#   3) GADEN_FILL_WIND=1 preprocess  -> re-fill ONLY the wind field (gas untouched)
#
# WHY: a FILLED wind field thins the baked plume (sparse gas -> robot wedges); an
# UNFILLED field gives ~55% zero-wind live obs (also wedges). The working config
# needs BOTH: gas baked on the sawtooth wind, THEN the wind field filled. Plain
# presim bakes gas on the (now fill-ON-by-default) filled wind -> sparse plume.
# Use THIS instead of presim whenever you need the dense plume for RL deploy.
#
# Usage: ./predensegas.sh   (or: alias predensegas='~/ros2_ws/src/base/scripts/predensegas.sh')
...
# Auto-detect workspace root (three levels up from this script's directory)
...
echo -e "${BOLD}${CYAN} ║   GADEN DENSE-GAS (3-step recipe)   ║${RESET}"
echo -e "${BOLD}${CYAN} ╚══════════════════════════════════════╝${RESET}"
echo ""
...
# ── Run the 3-step dense-gas recipe per sim ──────────────────────────────────────
total=${#sim_list[@]}
current=0

echo ""
echo -e "${BOLD}${GREEN} ╔══════════════════════════════════════╗${RESET}"
echo -e "${BOLD}${GREEN} ║       Starting DENSE-GAS bake        ║${RESET}"
echo -e "${BOLD}${GREEN} ╚══════════════════════════════════════╝${RESET}"
...
for sim in "${sim_list[@]}"; do
    ...
    # Delegate to the 3-step recipe script (single source of truth).
    "$WS_ROOT/generate_dense_gas.sh" "$scenario" "$config" "$sim"
    status=$?
    if [ $status -ne 0 ]; then
        echo -e "\n  ${RED}${BOLD}ERROR:${RESET} dense-gas recipe failed for $sim (exit $status)"
        exit $status
    fi
