...
def _log_sum_exp(log_w):
    """Numerically stable log-sum-exp."""
    max_log = np.max(log_w)
    if not np.isfinite(max_log):
        return -np.inf
    return max_log + np.log(np.sum(np.exp(log_w - max_log)))
...
def _log_normalize(log_w):
    ...
    if not np.isfinite(lse):
        return np.full_like(log_w, -np.log(len(log_w)))
    return log_w - lse

def _get_weights(log_w):
    """Convert log-weights to normalized probability weights."""
    log_w_norm = _log_normalize(log_w)
...
class ParticleFilter:
    ...
    Internally uses log-space weights to prevent floating-point underflow in long runs.
    """
    def __init__(self, num_particles: int, search_bounds: dict[str, list[float]],
        ...
        # Log-space weights: uniform initialization
    ...
    @property
    def weights(self):
        """Get normalized probability weights (for external consumers)."""
        return _get_weights(self.log_weights)

    @weights.setter
    def weights(self, value):
        """Set weights from probability space (for backward compatibility)."""
        value = np.asarray(value, dtype=float)
        value = np.maximum(value, 1e-300)
        self.log_weights = np.log(value)
        self.log_weights = _log_normalize(self.log_weights)
    ...
    def update(self, measurement: float, sensor_position: tuple[float, float], skip_resample: bool = False):
        ...
        # Step 2: Update log-weights by adding log-likelihoods
        log_likelihoods = np.log(np.maximum(likelihoods, 1e-300))
        self.log_weights += log_likelihoods

        # Step 3: Normalize log-weights
        lse = _log_sum_exp(self.log_weights)
        if np.isfinite(lse):
            self.log_weights -= lse
        else:
            # All weights collapsed — reinitialize
            print("Warning: All log-weights are -inf. Reinitializing particles.")
            self.particles = self._initialize_particles()
            self.log_weights = np.full(self.N, -np.log(self.N))
            return
    ...
    def _effective_sample_size(self):
        """Compute effective sample size N_eff ≈ 1 / Σ(wⁱ)² using log-weights."""
        w = self.weights  # converts from log-space
    ...
    def get_estimate(self):
        ...
        estimate = {
            ...
            'Q': np.sum(w * self.particles[:, 2])
        }

        x_var = np.sum(w * (self.particles[:, 0] - estimate['x']) ** 2)
        y_var = np.sum(w * (self.particles[:, 1] - estimate['y']) ** 2)
        Q_var = np.sum(w * (self.particles[:, 2] - estimate['Q']) ** 2)
    ...
    def get_entropy(self):
        """Compute Shannon entropy of particle distribution using log-weights directly."""
        w = self.weights  # converts from log-space
        log_w_norm = _log_normalize(self.log_weights)
        # H = -Σ w * log(w), using log_w directly avoids log(exp(x)) roundtrip
        mask = w > 1e-15
        entropy = -np.sum(w[mask] * log_w_norm[mask])
        return entropy
    ...
    def compute_hypothetical_entropy(self, measurement: int, sensor_position: tuple[float, float]):
        ...
        # Work in log-space for hypothetical update
        log_lik = np.log(np.maximum(likelihoods, 1e-300))
        hyp_log_weights = self.log_weights + log_lik

        lse = _log_sum_exp(hyp_log_weights)
        if not np.isfinite(lse):
            return np.log(self.N)

        hyp_log_weights_norm = hyp_log_weights - lse
        hyp_weights = np.exp(hyp_log_weights_norm)

        mask = hyp_weights > 1e-15
        entropy = -np.sum(hyp_weights[mask] * hyp_log_weights_norm[mask])
    ...
    def predict_measurement_probability(self, sensor_position, binary_value=None):
        ...
        w = self.weights  # converts from log-space
        level_probs = bin_probs_per_particle @ w
    ...
    def compute_expected_entropy(self, sensor_position: tuple[float, float]) -> float:
        ...
        # Shape: (Num_Bins,)
        bin_total_probs = bin_probs_matrix @ w
        ...
        posterior_weights = (bin_probs_matrix * w[None, :]) / safe_denominators
