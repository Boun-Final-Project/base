"""
Standalone RRT-Infotaxis with IGDM (Indoor Gaussian Dispersion Model).

Implements the complete measure-plan-move loop from the paper.
No ROS2 or GADEN required.
...
class RRTInfotaxisIGDM:
    ...
    def __init__(self, sigma_m=1.0):
        ...
        # Visualization
        self.visualizer = StepVisualizer(output_dir="igdm_steps", igdm_model=self.igdm)
    ...
    def run(self):
        ...
        print("=" * 70)
        print("RRT-INFOTAXIS WITH IGDM (Measure-Plan-Move Loop)")
        print("=" * 70)
        print(f"Environment: {self.room_width}m × {self.room_height}m (empty)")
        print(f"True source: {self.true_source} (Q={self.true_Q})")
        print(f"Robot start: {self.robot_start}")
        print(f"Algorithm parameters:")
        print(f"  - IGDM sigma_m: {self.sigma_m}m (Dijkstra distance)")
    ...
    def visualize_final(self, filename='rrt_infotaxis_igdm_result.png'):
        ...
        ax1.set_title('RRT-Infotaxis Trajectory (IGDM)', fontsize=12, fontweight='bold')
        ax1.set_xlabel('X (m)')
        ax1.set_ylabel('Y (m)')
