"""
Adaptive RRT-Infotaxis with IGDM (Indoor Gaussian Dispersion Model) with Multiple Rooms.
...
class AdaptiveRRTInfotaxisIGDMRoomsDiscrete:
    """Adaptive RRT-Infotaxis with IGDM and 5-level discrete sensor for multi-room building.
    ...
    def run(self):
        """Run the full Adaptive RRT-Infotaxis algorithm with discrete sensor."""
        self.log("=" * 70)
        self.log("ADAPTIVE RRT-INFOTAXIS WITH IGDM + DISCRETE SENSOR + EXPLORATION PENALTY")
    ...
    def visualize_final(self, filename='adaptive_rrt_infotaxis_igdm_rooms_discrete_result.png'):
        ...
        ax1.set_title('Adaptive RRT-Infotaxis Trajectory (IGDM + Discrete Sensor)', fontsize=12, fontweight='bold')
        ax1.set_xlabel('X (m)')
        ax1.set_ylabel('Y (m)')
        ...
        if self.estimates:
            ...
            info_text = "ADAPTIVE RRT-INFOTAXIS RESULTS (MULTI-ROOM + DISCRETE)\n"
            info_text += "="*40 + "\n\n"
...
if __name__ == "__main__":
    ...
    infotaxis = AdaptiveRRTInfotaxisIGDMRoomsDiscrete(sigma_m=1.0, logger=logger)
    infotaxis.run()
    final_png = log_dir / "adaptive_rrt_infotaxis_igdm_rooms_discrete_result.png"
