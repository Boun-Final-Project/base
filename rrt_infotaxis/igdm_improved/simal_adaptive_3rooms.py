...
UPDATED: Multi-Room Layout (3 Rooms + Hallway).
...
5. VISUALIZE: Save step with RRT tree overlay
6. MOVE: Navigate to next position
"""
...
class RRTInfotaxisIGDM:
    """Standalone RRT-Infotaxis with IGDM and Discrete Sensor on Multi-Room Map."""
    ...
    def __init__(self, sigma_m=1.0, robot_start=None):
        ...
        # Source inside Room 1 (Top-Left), Robot in Central Hallway
        ...
        # Initialize with Standard Parameters (Adaptive logic will update this)
    ...
    def is_near_obstacle(self, position, radius=0.75):
        ...
        Includes Fix for Slice Indices TypeError.
        """
    ...
    def run(self):
        ...
        print("=" * 70)
        print("RRT-INFOTAXIS (MULTI-ROOM) + DISCRETE SENSOR + OBSTACLE ADAPTIVE")
        print("=" * 70)
        print(f"Environment: {self.room_width}m × {self.room_height}m")
        print(f"Room Layout: 3 Rooms + Hallway")
        print(f"True Source: {self.true_source}")
        print(f"Planning: RRT with 4-way initial spread")
    ...
    def visualize_final(self, filename='rrt_infotaxis_igdm_discrete_multiroom_adaptive_result.png'):
        ...
        ax1.set_title('RRT-Infotaxis Trajectory (Multi-Room + Obstacle Adaptive)', fontsize=12, fontweight='bold')
        ax1.set_xlabel('X (m)')
        ax1.set_ylabel('Y (m)')
