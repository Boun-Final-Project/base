"""
Directional Planner with IGDM (Indoor Gaussian Dispersion Model) on Large Map with Discrete Sensor.

Implements the complete measure-plan-move loop with exploration penalty
to discourage revisiting recent areas. Uses 4-directional simple planning strategy
and 5-level DISCRETE SENSOR.
...
class DirectionalPlannerIGDMDiscreteLargeMap:
    """Directional Planner with IGDM and discrete sensor on large map."""
    ...
    def log_all_path_evaluations(self, debug_info, best_idx):
        ...
        -----------
        debug_info : dict
            Debug information from planner
        best_idx : int
    ...
    def run(self):
        ...
        self.log("=" * 70)
        self.log("DIRECTIONAL PLANNER WITH IGDM + DISCRETE SENSOR + EXPLORATION PENALTY (Large Map)")
        self.log("=" * 70)
        ...
        self.log(f"  - Planner: 4 directional paths (Forward, Backward, Left, Right)")
    ...
    def visualize_final(self, filename='rrt_infotaxis_igdm_improved_large_map_directional_result.png'):
        ...
        ax1.set_title('Directional Planner Trajectory (IGDM + Discrete Sensor + Exploration Penalty, Large Map)', fontsize=12, fontweight='bold')
        ax1.set_xlabel('X (m)')
        ax1.set_ylabel('Y (m)')
        ...
        if self.estimates:
            ...
            info_text = "DIRECTIONAL PLANNER RESULTS\n"
            info_text += "="*40 + "\n\n"
...
if __name__ == "__main__":
    ...
    planner = DirectionalPlannerIGDMDiscreteLargeMap(sigma_m=1.0, logger=logger)
    planner.run()
    final_png = log_dir / "rrt_infotaxis_igdm_improved_large_map_directional_result.png"
    planner.visualize_final(str(final_png))
