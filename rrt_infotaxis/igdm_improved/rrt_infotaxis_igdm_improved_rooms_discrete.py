...
class RRTInfotaxisIGDMRoomsDiscrete:
    """Standalone RRT-Infotaxis with IGDM and 5-level discrete sensor for multi-room building."""
    ...
    def __init__(self, sigma_m=1.0, logger=None):
        ...
        viz_dir = Path("/home/hdd/akademia/cmpe/final-project/week-10/igdm_improved_rooms_discrete_steps")
    ...
    def run(self):
        ...
        self.log("=" * 70)
        self.log("RRT-INFOTAXIS WITH IGDM + DISCRETE SENSOR + EXPLORATION PENALTY")
    ...
    def visualize_final(self, filename='rrt_infotaxis_igdm_improved_rooms_discrete_result.png'):
        ...
        ax1.set_title('RRT-Infotaxis Trajectory (IGDM + Discrete Sensor + Exploration Penalty)', fontsize=12, fontweight='bold')
        ax1.set_xlabel('X (m)')
        ax1.set_ylabel('Y (m)')
        ...
        if self.estimates:
            ...
            info_text = "RRT-INFOTAXIS RESULTS (MULTI-ROOM + DISCRETE SENSOR)\n"
            info_text += "="*40 + "\n\n"
...
if __name__ == "__main__":
    ...
    log_file = log_dir / "rrt_infotaxis_igdm_improved_rooms_discrete.log"
    logger = setup_logging(str(log_file))

    # Run with default sigma_m=1.0
    infotaxis = RRTInfotaxisIGDMRoomsDiscrete(sigma_m=1.0, logger=logger)
    infotaxis.run()
